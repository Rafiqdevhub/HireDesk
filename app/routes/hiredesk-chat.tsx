  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen bg-linear-to-br from-slate-900 via-slate-800 to-slate-900">
        ...
        {/* Assistant docked on the left bottom corner */}
        <aside className="fixed left-4 bottom-4 z-50 w-[360px] max-w-[calc(100%-2rem)] md:max-w-sm">
          <div className="rounded-2xl border border-slate-700/70 bg-slate-900/80 backdrop-blur-xl shadow-2xl">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800/70">
              <div>
                <p className="text-[11px] uppercase tracking-[0.18em] text-indigo-200/80 font-semibold">
                  AI Assistant
                </p>
                <p className="text-sm text-white font-semibold">
                  HireDesk live thread
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setMessages([])}
                  className="text-[11px] text-slate-300 hover:text-white underline decoration-dotted underline-offset-4"
                >
                  Clear
                </button>
                <button
                  onClick={() => setAssistantOpen((prev) => !prev)}
                  className="h-8 w-8 rounded-full bg-slate-800/80 border border-slate-700/70 text-slate-200 hover:text-white hover:border-slate-500 transition-colors"
                  aria-label={
              ...
              <div className="max-h-[420px] overflow-y-auto space-y-3 p-4">
                {messages.length === 0 && (
                  <div className="text-sm text-slate-400">
                    No interactions yet. Submit a prompt to see HireDesk
                    responses.
                  </div>
                )}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {message.role === "user" ? "You" : "HireDesk"}
                        </span>
                        <span className="text-[10px] text-slate-400 bg-slate-800/80 rounded-full px-2 py-0.5">
                          {message.queryType}
                        </span>
                      </div>
